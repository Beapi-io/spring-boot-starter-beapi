 ...
 */
	...
	private String getCreateData(){
		String version = "1"
		def tmp = apiCacheService.getApiCache(realClassName)
		def apiObject =tmp[version].create
		...
		createList.each(){
			if(it!="id" && it!="version") {
				output += realClassName + ".set${it.capitalize()}(this.params.get(\"" + it + "\"));\n"
			}
		}
	...
	private String getUpdateData(){
		String version = "1"
		def tmp = apiCacheService.getApiCache(realClassName)
		def apiObject =tmp[version].update
