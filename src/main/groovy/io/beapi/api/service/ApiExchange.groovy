...
import jakarta.servlet.forward.*
...
abstract sealed class ApiExchange permits ExchangeService, BatchExchangeService, ChainExchangeService, TraceExchangeService {
    ...
    * validating here to better handle routing (filter is 'onceperrequest')
    ...
    protected boolean validateMethod(){
        ...
        if(this.apiObject['method'].toUpperCase().equals(this.method)){
            result = true
        }
        return result
    }
        ...
        switch(responseFileType){
            ...
            case 'XML':
                try{
                    ...
                    test = (responseBody != null) ? xmlMapper.writeValueAsString(responseBody) : '{}'
                }catch(Exception e){
                    throw new Exception("[ApiExchange :: parseBodyByFiletype] : "+e)
                }
                break;
